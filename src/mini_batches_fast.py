...
class mini_batches_fast(mini_batches_code):
    ...
    # @profile
    # def get_batch(self):
    #     # Sample without replacement for current epoch
    #     sample_size = min(self.sample_size, len(self.remaining_papers))
    #     random_sample = random.sample(list(self.remaining_papers), sample_size)
    #     for paper in random_sample:
    #         self.remaining_papers.remove(paper)

    #     # Gather edges from sampled papers
    #     edges = []
    #     for src in random_sample:
    #         for tgt in self.citation_dict[src]:
    #             edges.append((src, tgt))

    #     if not edges:
    #         return torch.empty((2, 0), dtype=torch.long, device=self.device), random_sample, []

    #     edges_tensor = torch.tensor(edges, dtype=torch.long, device=self.device).T  # shape [2, num_edges]

    #     # Get all target nodes (cited papers) for next round
    #     l_next = list(set(tgt for _, tgt in edges))

    #     # Reset epoch if all papers have been seen
    #     if len(self.remaining_papers) == 0:
    #         self.remaining_papers = set(self.all_papers)

    #     return edges_tensor, l_next, random_sample
    ...
    #     unequal_mask = (r_venue != j_venue).flatten()
