...
dm, ul, random_sample = mini_b.data_matrix()
dm = dm[dm[:,4]!=4]
...
for i in dm[:, 2].tolist():
    tensor_value = paper_dict[i]  # Access the tensor from paper_dict
...
paper_dict[random_sample[0]] = torch.nn.Parameter(new_embedding.clone().detach())
print(paper_dict[random_sample[0]])
new_optimizer = torch.optim.Adam([paper_dict[random_sample[0]]], lr=learning_rate)
