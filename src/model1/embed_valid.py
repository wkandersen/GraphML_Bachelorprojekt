...
paper_dict = torch.load('checkpoint/checkpoint_iter_2_25_epoch_1.pt')
...
#         tensor_value = paper_dict['collected_embeddings']['paper'][i]  # Access the tensor from paper_dict
...
#     new_embedding = nn.Parameter(mean_tensor)
    ...
    new_embedding = torch.nn.Embedding(1,2).weight

    # Now mean_tensor holds the element-wise mean of all tensors
    print(f"Element-wise mean tensor: {mean_tensor}")
    
    paper_dict['collected_embeddings']['paper'][random_sample[0]] = new_embedding
    print(paper_dict['collected_embeddings']['paper'][random_sample[0]])
    new_optimizer = torch.optim.Adam([paper_dict['collected_embeddings']['paper'][random_sample[0]]], lr=learning_rate)
    ...
    for epoch in range(num_epochs):
        ...
        loss = loss_function.compute_loss(paper_dict['collected_embeddings'], final)  # Compute loss
