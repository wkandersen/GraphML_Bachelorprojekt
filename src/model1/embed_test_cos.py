...
from Packages.loss_function_cos import LossFunction
...
def evaluate_sample(paper_emb, venue_embeddings, venue_ids, true_label):
    cos_sim = F.cosine_similarity(venue_embeddings, paper_emb, dim=1)
    logits = torch.sigmoid(alpha - cos_sim)
...
check = torch.load(f'checkpoint/2025-06-13_13-26-41/checkpoint_iter_64_{embedding_dim}_50_epoch_18_weight_0.1_with_optimizer.pt')
...
run = wandb.init(
    project="Forsvar",
    name=f"test_cos_{embedding_dim}_{datetime.now():%Y-%m-%d_%H-%M-%S}",
    config={
...
torch.save(predictions, os.path.join(save_dir, f'pred_dict_{embedding_dim}_valid.pt'))
...
torch.save(filtered_paper_dict, os.path.join(save_dir, f'valid_embeddings_dict_{embedding_dim}.pt'))
