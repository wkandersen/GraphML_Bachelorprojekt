...
# Venue embeddings
embed = torch.nn.Embedding(len(venues_values), embedding_dim+2)
...
# Step 2: Create learnable 2D embedding
...
# Step 3: Build collected embeddings (4D: learnable + frozen)
for pid, emb in zip(paper_c_paper_train.flatten(), embeddings):
    paper_id = pid.item()
    emb2d = dict2[paper_id]  # [2]
    emb4d = torch.cat((emb.detach().clone(), emb2d.detach().clone()), dim=-1)  # → [4]
    collected_embeddings['paper'][paper_id] = emb4d
